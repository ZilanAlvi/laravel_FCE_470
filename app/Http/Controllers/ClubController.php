...
public function bidStatus()
    ...
    public function send_match_request(Request $request)
    {
        ...
        // Check if the authenticated user's club exists
        $authenticatedUserClub = Club::where('user_id', auth()->user()->id)->first();
    
        if (!$authenticatedUserClub) {
            return redirect()->back()->with('error', 'Your club does not exist.'); // Handle the case where the user's club does not exist
        }
    
        $matchRequest = new MatchRequest;
        $matchRequest->club_id = $authenticatedUserClub->id; // Assign the club_id from the user's club
